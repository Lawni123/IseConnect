...
public class NptelViewDao {
    ...
    public List<NptelCertification> getNptelCertifications(String designation, LocalDate startDate, LocalDate endDate) 
        ...
        String query = "SELECT n.id, u.name AS user_name, u.usn, n.course_name, n.course_duration, " +
                "n.certificate_url, n.uploaded_at " +
                "FROM nptel_certifications n " +
                "JOIN " + tableName + " u ON n.user_id = u." + idColumn + " " +
                "WHERE n.designation = ? " +
                "AND DATE(n.uploaded_at) BETWEEN ? AND ? " +
                "ORDER BY n.uploaded_at DESC";

 try (PreparedStatement stmt = connection.prepareStatement(query)) {
     stmt.setString(1, designation.toLowerCase());
     stmt.setDate(2, Date.valueOf(startDate)); // Start date
     stmt.setDate(3, Date.valueOf(endDate));   // End date
     ...
     while (rs.next()) {
         certifications.add(new NptelCertification(
             rs.getInt("id"),
